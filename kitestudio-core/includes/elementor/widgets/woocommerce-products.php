...
class Kite_Woocommerce_Products_Widget extends \Elementor\Widget_Base {
	...
	public function get_name() {
		return 'kite-woocommerce-products';
	}
	...
	public function get_title() {
		return esc_html__( 'Woocommerce Products', 'kitestudio-core' );
	}
	...
	public function get_icon() {
		return 'eicon-products kite-element-icon';
	}
	...
	protected function register_controls() {
		...
		if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) { // check is gallery plugin is active or not
			foreach ( $terms as $term_id => $slug ) {
					$product_cats[ $slug ] = get_term($term_id)->name;
			}
		}
		$cat_args['taxonomy'] = 'product_tag';
		$tag_terms = get_terms( $cat_args );
		if ( ! empty( $tag_terms ) && ! is_wp_error( $tag_terms ) ) { // check is gallery plugin is active or not
			foreach ( $tag_terms as $term_id => $slug ) {
					$product_tags[ $slug ] = get_term($term_id)->name;
			}
		}
		...
		$this->start_controls_section(
			'content_section',
			array(
				'label' => esc_html__( 'Woocommerce Products', 'kitestudio-core' ),
	...
	protected function render() {
		...
		echo kite_woocommerce_products( $args );
	...
	public function kite_generate_shortcode( $settings, $return_args = false ) {
		...
		$shortcode = '[woocommerce_products ';
