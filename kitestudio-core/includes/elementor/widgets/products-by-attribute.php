...
class Kite_Woocommerce_Products_By_Attributes_Widget extends \Elementor\Widget_Base {

	/**
	 * Get widget name.
	 *
	 * Retrieve Woocommerce Products widget name.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function get_name() {
		return 'kite-woocommerce-products-by-attribute';
	}

	/**
	 * Get widget title.
	 *
	 * Retrieve Woocommerce Products widget title.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function get_title() {
		return esc_html__( 'Woocommerce Products By Attributes', 'kitestudio-core' );
	}
	...
	public function get_icon() {
		return 'eicon-product-breadcrumbs kite-element-icon';
	}
	...
	public function get_keywords() {
		return array( 'woocommerce', 'shop', 'products', 'attributes' );
	}

	/**
	 * Get widget categories.
	 *
	 * Retrieve the list of categories the Woocommerce Products widget belongs to.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	protected function register_controls() {
		...
		if ( ! empty( $attributes ) ) {
			foreach ( $attributes as $key => $attribute ) {
				$attributes_name[ $attribute->attribute_name ] = $attribute->attribute_label;
			}
		}
		...
		$this->start_controls_section(
			'content_section',
			array(
				'label' => esc_html__( 'General Options', 'kitestudio-core' ),
		...
		$this->add_control(
			'attribute',
			array(
				'label'       => esc_html__( 'Attribute', 'kitestudio-core' ),
				'label_block' => true,
				'type'        => \Elementor\Controls_Manager::SELECT,
				'options'     => $attributes_name,
			)
		);
		foreach ( $attributes_name as $attribute_name => $attribute_label ) {
			$terms = get_terms( 'pa_' . $attribute_name, $attr_args );

			if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
				$tax_terms = array();
				foreach ( $terms as $taxonomy ) {
						$tax_terms[ $taxonomy->slug ] = $taxonomy->name;
				}
				$this->add_control(
					'filter_' . $attribute_name,
					array(
						'label'       => esc_html__( 'Filters', 'kitestudio-core' ),
						'label_block' => true,
						'multiple'    => true,
	...
	protected function render() {
		...
		echo kite_product_attribute( $args );
	}
	...
	public function kite_generate_shortcode( $settings, $return_args = false ) {
		...
		if ( ! empty( $settings['attribute'] ) ) {
			$filters = $settings[ 'filter_' . $settings['attribute'] ];
			if ( is_array( $filters ) ) {
				$fitler = implode( ',', $filters );
			} else {
				$fitler = $filters;
			}
		}
		...
		$args = array(
			...
			'catalog_mode'           => $settings['catalog_mode'] === 'enable' ? '' : true,
		);
