...
class Wishlist extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Header - Wishlist', 'kitestudio-core' );
	}
	...
	public function get_icon() {
		return 'eicon-heart-o kite-element-icon';
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'content_section',
			array(
				'label' => esc_html__( 'Wishlist', 'kitestudio-core' ),
		...
		$this->add_control(
			'icon',
			array(
				'label'   => esc_html__( 'Wishlist Icon', 'kitestudio-core' ),
		...
		$this->add_control(
			'icon_subtitle',
			array(
				'label' => esc_html__( 'Wishlist Icon Subtitle', 'kitestudio-core' ),
		...
		$this->add_control(
			'title',
			array(
				'label' => esc_html__( 'Wishlist Icon Title', 'kitestudio-core' ),
				'type'  => Controls_Manager::TEXT,
			)
		);
		...
		$this->add_control(
			'show_items_number',
			array(
				'label'        => esc_html__( 'Show Total Items In Wishlist', 'kitestudio-core' ),
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '',
				'selectors' => array(
					'{{WRAPPER}} .kt-header-button .kt-icon-container .kt-badge' => 'color: {{VALUE}};',
				),
			)
		);
	...
	protected function render() {
		...
		$wishlist_url = ! empty( $yith_wcwl->get_wishlist_url() ) ? $yith_wcwl->get_wishlist_url() : '#';

		?>
		<div class="kt-header-button kt-wishlist">
			<a class="hd-btn-link" href="<?php echo esc_url( $wishlist_url ); ?>"></a>
				<div class="kt-icon-container">
					...
					<?php if ( $settings['show_items_number'] == 'yes' && ! empty( $settings['show_items_number_badge'] ) && $settings['show_items_number_badge'] == 'yes' ) { ?>
						<span class="kt-badge kt-total "><?php echo esc_html( $count ); ?></span>
					<?php } ?>
				</div>
				<div class="kt-meta-texts">
					...
					<?php if ( $settings['show_items_number'] == 'yes' && $settings['show_items_number_badge'] != 'yes' ) { ?>
						<span class="kt-total-items"><span class="kt-total"><?php echo esc_html( $count ); ?></span> <?php echo esc_html__( 'Items', 'kitestudio-core' ); ?></span>
					<?php } ?>
				</div>
		</div>
		<?php
	...
	protected function content_template() {
		?>
		<div class="kt-header-button kt-whishlist">
			<div class="kt-icon-container">
				<# if ( settings.icon.library == 'svg' ) { #>
